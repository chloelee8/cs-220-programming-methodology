...
export function stableMatchingRunOracle(makeStableMatchingTrace) {
    for (let i = 0; i < NUM_TESTS; ++i) {
        ...
        const companyMatch = Array.from({ length: N }, () => null);
        const candidateMatch = Array.from({ length: N }, () => null);
        const companyNextOffer = Array.from({ length: N }, () => 0); // index of next candidate to propose to
        const candidateNextOffer = Array.from({ length: N }, () => 0); // index of next company to propose to
        for (const offer of trace) {
            ...
            if (fromCo) {
                ...
                assert(receiver === expectedReceiver, `Company ${proposer} should propose to ${expectedReceiver}, not ${receiver}`);
                ...
                assert(receiver === expectedReceiver, `Candidate ${proposer} should propose to ${expectedReceiver}, not ${receiver}`);
        ...
        for (let c = 0; c < N; c++) {
            ...
            if (candidate !== null)
                finalHires.push({ company: c, candidate });
        }
        ...
        for (const hire of out) {
            ...
            assert(match === hire.candidate, `Hire mismatch: company ${hire.company} should be matched with ${match ?? "null"}, got ${hire.candidate}`);
        }
    }
}
