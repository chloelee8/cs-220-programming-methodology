...
describe("byParity", () => {
  ...
  it("should treat 0 as an even number", () => {
    const evenFunc = (n: number) => n * 2;
    const oddFunc = (n: number) => n + 2;
  ...
  it("should only call the correct function", () => {
    let e_call = 0;
    let o_call = 0;
    const evenFunc = (n: number) => {
      e_call++;
      return n + 1;
    };

    const oddFunc = (n: number) => {
      o_call++;
      return n + 2;
    };
...
describe("wageChange", () => {
  ...
  it("should correctly calculate wage increase over years", () => {
    const calcNew = (yr: number, prevWage: number) => prevWage + 5;
